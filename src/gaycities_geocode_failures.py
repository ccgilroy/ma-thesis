...
# check_responses = [r for r in census_responses if r['id'] in [143, 387, 489, 588]]
#
# for r in census_responses:
#     if r['id'] in [143, 387, 489, 588]:
...
for r in gm_responses:
    r['coordinates'] = str(r['lng']) + ',' + str(r['lat'])

coordinates_df = pd.DataFrame(gm_responses, columns=['id', 'coordinates'])
geographies_df = pd.DataFrame(census_responses, columns=['id', 'state', 'county', 'tract', 'block', 'GEOID'])

gaycities_geocoded_by_coordinates = (
    gaycities_new_addresses.
